...
start-db:
	...
	@docker run --name ${DB_DOCKER_NAME} -d \
		--env-file ${CONFIG_FILE_PATH} ${DB_IMAGE}
	@sleep 1
...
query-db:
	@docker exec -it ${DB_DOCKER_NAME} psql -d process_db -U ${DB_ROOT_USER}

query-persons-db:
	@docker exec ${DB_DOCKER_NAME} psql -d process_db -U ${DB_ROOT_USER} \
